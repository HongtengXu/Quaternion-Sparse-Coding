...
if (param.preserveDCAtom>0)
    FixedDictionaryElement = 1/(2*sqrt(size(Data,1)))*ones(size(Data,1),1,4);%能量归一化
else
...
for iterNum = 1:param.numIteration
    ...
    if (param.errorFlag==0)
        ...
        output.totalerr2(iterNum) = sqrt(sum(sum(sum(Data - tempout).^2)))/size(Data,2);
        disp(['Iteration   ',num2str(iterNum-1),'   Total error is: ',num2str(output.totalerr2(iterNum))]);        
    ...
    if (iterNum>=1 & param.displayProgress)
        if (param.errorFlag==0)
            tempout = Qmult([FixedDictionaryElement,Dictionary],CoefMatrix);
            output.totalerr(iterNum) = sqrt(sum(sum(sum(Data - tempout).^2))/prod(size(Data)));
           % disp(['Iteration   ',num2str(iterNum),'   Total error is: ',num2str(output.totalerr(iterNum))]);
        else
            output.numCoef(iterNum-1) = length(find(CoefMatrix))/(size(Data,2)*4);
...
CoefMatrix1 = CoefMatrix(:,:,1).^2 + CoefMatrix(:,:,2).^2  + CoefMatrix(:,:,3).^2 + CoefMatrix(:,:,4).^2;
for jj=1:1:K,
    if max(G(jj,:))>T2 || length(find(abs(CoefMatrix1(jj,:,:))>1e-5))<=T1 ,%字典的第jj列与某一列相关性>1，或者使用字典第jj列的系数矩阵第j行的非零个数少于3
        [val,pos]=max(Er);
